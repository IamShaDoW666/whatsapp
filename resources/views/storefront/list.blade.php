...
@foreach($products as $key => $items)
    <div class="collection-lists collection-items {{ ($loop->iteration != 1) ? 'd-none' : ''  }} {{$loop->iteration-1}}{!!str_replace(' ', '_', __($key))!!} product_tableese">
        @foreach($items as $product)
            <div class="collection-list product_item" data-name="{{$product->name}}">
                <div class="collection-left">
                    <div class="collection-img">
                        <a href="#" data-size="lg" data-url="{{ route('store.product.product_view',[$store->slug,$product->id]) }}" data-title="{{$product->name}}" data-ajax-popup="true">
                ...
                <div class="collection-right">
                    ...
                    @else
                        <div class="item-spin">
                            <p class="price-spin">{{\App\Utility::priceFormat($product->price)}}</p>
                        </div>
                    @endif
                    <a href="#" data-size="lg" data-url="{{ route('store.product.product_view',[$store->slug,$product->id]) }}" data-ajax-popup="true" class="product_details_icon float-right mt-1"><i class="fas fa-eye product_details_active text_color"></i></a>
                    @if($product->enable_product_variant == 'on')
                        <a href="#" class="btn btn-addcart" data-size="lg" data-url="{{ route('store-variant.variant',[$store->slug,$product->id])}}" data-ajax-popup="true" data-title="{{__('Add Variant')}}">{{__('ADD TO CART')}}
