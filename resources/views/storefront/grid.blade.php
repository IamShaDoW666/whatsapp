...
<div>
    @foreach($products as $key => $items)
        <div class="row collection-items {{ ($loop->iteration != 1) ? 'd-none' : ''  }} {{$loop->iteration-1}}{!!str_replace(' ', '_', __($key))!!} product_tableese">
            @foreach($items as $product)
                <div class="col-sm-6 col-md-6 col-xl-3 product_item" data-name="{{$product->name}}">
                    <div class="product-card">
                        <a href="#" data-size="lg" data-url="{{ route('store.product.product_view',[$store->slug,$product->id]) }}" data-title="{{$product->name}}" data-ajax-popup="true" class="product_details_icon float-right"><i class="fas fa-eye product_details_active text_color"></i></a>
